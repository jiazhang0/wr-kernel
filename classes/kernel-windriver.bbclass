...
addtask kernel_kfeat before do_configure after do_kernel_configme
do_kernel_kfeat[dirs] = "${S} ${B}"
do_kernel_kfeat() {
	set +e

	procfam="unknown procfam"
	variant="unknown variant"

	# rather than coding the architecture and variant detection within 
	# the kernel code itself, we'll check the generated .config for 
	# arch and variant information and feed it to kfeat. If it is done 
	# here, it is simple enough to override this routine vs patching the
	# kernel to fix issues

	# The case statements are *not* to be board specific. They must be
	# arch generic, and only use values that can be found within the 
	# generated kernel .config and not pulled from the build system
	# configuration, since this code may eventually move back into the
	# kernel or be autodetected by tools once mainline-only values are
	# the norm.

	# The statements should be ordered from generic to slightly more
	# specific, to allow a decent baseline and then more specific overrides
	# as appropriate.

	# if there are issues with this table, simple overide this routine
	# in another layer and dump custom values.
	case ${ARCH} in
		arm)
			grep -q "^CONFIG_CPU_32v4" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="armv4"
			fi
			grep -q "^CONFIG_CPU_32v4T" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="armv4t"
			fi
			grep -q "^CONFIG_CPU_32v5" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="armv5"
			fi
			grep -q "^CONFIG_CPU_32v6" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="armv6"
			fi
			grep -q "^CONFIG_CPU_32v7" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="armv7"
			fi
			;;
		x86)
			grep -q "^CONFIG_MATOM" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="atom"
			fi
			grep -q "^CONFIG_MCORE2" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="core"
			fi
			;;
		powerpc)
			grep -q "^CONFIG_PPC_85xx" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="85xx"
			fi
			grep -q "^CONFIG_PPC_8xx" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="8xx"
			fi
			grep -q "^CONFIG_44x" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="44x"
			fi
			grep -q "^CONFIG_PPC_E500MC" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="qoriq_p4xxx"
			fi
			grep -q "^CONFIG_6xx" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="6xx"
			fi
			;;
		mips)
			grep -q "^CONFIG_CPU_MIPS32_R1" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="mips32r1"
			fi
			grep -q "^CONFIG_CPU_MIPS32_R2" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="mips32r2"
			fi
			grep -q "^CONFIG_CAVIUM_OCTEON" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="cn5xxx"
				grep -q CONFIG_CAVIUM_CN63XXP1 ${B}/.config
				if [ $? -eq 0 ]; then
					procfam="cn6xxx"
				fi
			fi
			grep -q "^CONFIG_NLM_XLR_BOARD" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="rmixlr"
			fi
			grep -q "^CONFIG_NLM_XLP_BOARD" ${B}/.config
			if [ $? -eq 0 ]; then
				procfam="rmixlp"
			fi
			;;
	...
	echo "[INFO] doing kernel kfeat generation: ${MACHINE} ${procfam} \"${variant}\""
	kfeat_bin=`which kfeat`
	if [ -n "$kfeat_bin" ]; then
		# generates wrs_sys_info.h for WRNOTE
		kfeat ${WORKDIR} ${S} ${LINUX_VERSION} ${MACHINE} "${procfam}" "${variant}"
		# Commit wrs_sys_info.h runtime change
		(
		cd ${S}
		git add ./include/linux/wrs_sys_info.h
		git commit -m "wrnote: Runtime change commit"
		cd -
		)
	fi
}
